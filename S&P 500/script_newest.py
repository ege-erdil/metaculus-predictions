...
yields_fwd = [np.prod(1 + yields[i:i+h]/100 + (prices[i+1:i+1+h]/prices[i:i+h] - 1)) for i in range(len(yields) - h)]

yields = yields[:-h]
yields = np.reshape(yields, (len(yields), 1))

#returns = 100 * ( ((prices[h:] * pce[h:] * yields_fwd)/(prices[:-h] * pce[:-h]))**(1/h) - 1)
returns = 100 * ( ((pce[h:] * yields_fwd)/(pce[:-h]))**(1/h) - 1)
...
model = sm.OLS(endog=returns[lag:], exog=ex[:len(ex) - lag])
res = model.fit()
...
q = res.get_prediction(exog=ex[:len(ex)-lag])
print(np.std(q.predicted_mean))

print(estimate(p.predicted_mean[0] + res.resid, num=1, tol=0.0001))

plt.plot(range(start, 2021-h-lag), q.predicted_mean, color="r")
plt.plot(range(start, 2021-h-lag), returns[lag:], color="b")
plt.show()
